...
const b = JSON.parse(fs.readFileSync(join(__dir,'boundaries_video.json'),'utf8'));
const TS = b.timestamps; const N = TS.length; const VW=1920, VH=1080;
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* Busy spinner — in the controls row only, never over the slide */
  ...
  /* Narrow embeds (phones): 39 dots wrap into rows and waste height + are too
...
<body>
  ...
  <script>
    ...
    // Rest = the PAUSED video frame at this slide's keyframe timestamp (no JPG → no
    // color-space/resolution mismatch; everything is the video).
    function settleOn(i){
    ...
    // Next: play the real transition from this slide to the next, then pause on the
    // next slide's keyframe.
    function next(){
      ...
      function go(){
        ...
        function watch(){
          ...
          // Stall watchdog: if time isn't advancing and we're not mid-seek, the
          // play() didn't take (autoplay hiccup / paused-after-jump) — re-kick it.
    ...
    // Busy spinner (controls row only) — show while the video is seeking (jumps)
    // or playing a transition; never overlays the slide.
...
fs.writeFileSync(join(__dir,'index.html'), html);
console.log('wrote video-ONLY viewer,', N, 'slides');
