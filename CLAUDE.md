...
**2026-06-29** - **Marker/timeline editor (v1.3.0) built from an HTML mockup, then ~10 live cycles surfaced what specs couldn't.** (1) **HoldDetector motion-diff is BLIND to fades on a dark bg** — the real ILS deck's cross-fades keep per-frame 32×18 diff under threshold, so the detector expanded each hold THROUGH the transition until it abutted the next (gap=1f): the timeline rendered as one purple bar AND `holdStart` expanded backward into the prior fade so Rest could land mid-animation (the exact bug). FIX = **Rest is the DP anchor verbatim** (settled frame — never expand backward), **Go = forward motion onset** from the anchor, **default ~15-frame transition band** when no motion is detectable. Proven only by a **headless harness on the REAL deck** — the synthetic test deck was too clean to expose gap=1f (a synthetic/mocked encode encodes your wrong assumption; the real fade-heavy deck is the oracle). (2) **macOS dev-launch double-window:** `osascript … activate "<AppName>"` resolves the name via LaunchServices to the INSTALLED `/Applications` copy (a stale v1.0.0) and launches it ALONGSIDE the `/tmp` dev build → two windows. **Launch by PATH only** (`open -n <path>`), never activate-by-name; verify the running path via `ps -p <pid> -o command=`. (Replace the stale `/Applications/KeynoteDeployer.app` v1.0.0 once v1.3.0 ships.) (3) **SwiftUI window grew past the monitor** — a phase's content (big preview + controls + timeline) had a tall min height and SwiftUI sizes the window to content's min. FIX = wrap the editor in a **ScrollView + pinned footer** (ScrollView min-height ≈0) so the window can't be forced past the screen; `.defaultSize` is only the open size (set 1100×600). (4) **Iterate UX in an HTML mockup with Edward BEFORE Swift** — a clickable play/freeze + playhead mockup settled layout A + the play-vs-freeze model in minutes instead of rebuild-the-app cycles; then build the chosen one 1:1. (5) Persistence = `MarkStore` keyed by deck fingerprint (frames+fps+size): saves on Encode & Deploy, auto-loads on re-drop of the SAME export; a re-export changes the key so stale marks never apply. (6) ffmpeg 8.1 here has **no `drawtext`** (no libfreetype) → draw labeled test slides with PIL; zsh arrays are 1-indexed AND don't word-split unquoted scalars (use `arr=(...)` + `"${arr[@]}"`). Branch `feat/timeline-editor`, 87 tests; iPhone gate + merge still pending. → `keynote-deployer.md`
**2026-06-28** - **Deck transitions stuttered / skipped-to-rest / froze-then-continued on the FIRST visit to a slide, but go-back-and-retry ALWAYS worked (v1.2.1).** Burned hours chasing wrong root causes — each "fix" was a symptom of ONE thing: **mid-playback network buffering.** The viewer seeks+plays a short transition per slide; on first visit that byte range isn't downloaded, so playback stalls mid-transition. Wrong theories tried & disproven, in order: (1) **rest point off-keyframe** (CRF16 slide-only keyframes → seek decodes a full GOP back) → set `REST_BIAS=0` to rest ON the keyframe → made it WORSE ("slides 8 & 9 became the same" — the derived `TS` is per-slide imperfect; a global bias can't fix placement, that's the marker-editor's job). (2) **GOP too long** → `-g 60` (2s) → stuttered WORSE (more keyframes ≠ smoother → NOT decode-depth). (3) **B-frames** → the smooth reference encode had `has_b_frames=0` (VideoToolbox) vs ffmpeg's 155 B-frames/300; `-bf 0` helped seeking but DIDN'T fix first-play. (4) Decode-cost & wall-clock-timing red herrings. THE TELL: "**this wasn't happening on the larger (82MB) file**" — that was a **browser-CACHE artifact** (the 82MB AVFoundation deck had been loaded many times earlier so it played from cache; every fresh CRF16 redeploy = fresh download = first-play stall). Bigger-file-smoother is impossible for a download problem → proved it's data-availability, not the encoder. **FIX: download the whole deck into an in-memory blob (`XHR responseType=blob` → `URL.createObjectURL` → `vid.src`, with a % progress bar; `<video>` has NO static `src`, set in JS; stream-fallback on error) and play from memory — zero mid-playback network.** First pass is now as smooth as a replay. Lessons: (a) "first time bad, retry good" == caching/buffering, FULL STOP — don't theorize encoder internals; (b) a "smooth vs stutter" A/B is INVALID if one side is browser-cached — bust the cache or blob-load both; (c) `'waiting'/'stalled'` events don't reliably fire for short range-fetch stalls, so a stall-aware wall-clock still skipped — only removing the network mid-play actually fixed it. Also shipped this release: ffmpeg now the PREFERRED encoder when installed (`FFmpegVideoEncoder.isAvailable()`; AVFoundation fallback) at **CRF16 -bf 0 -g 60** (~60% smaller decks, 82→30MB); `navBusy` re-entrancy lock (fast Next-mash can't race the paint micro-play into the next transition); `finish()` force-paints the rest frame (kills the dim→bright flash). v1.2.1 notarized + shipped. The per-slide rest-frame placement is STILL imperfect (derivation) — deferred to the planned marker-timeline editor (1.3.0) where the marker IS both the rest point and the forced keyframe.
